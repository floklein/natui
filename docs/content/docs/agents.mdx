...
the skill teaches the workflow, and the documentation supplies the current
...
3. Inspect the current checkout before editing. NatUI is in alpha. The
   registry package downloads a prebuilt native host on first launch; inside
   the framework repository, the host builds from source.
