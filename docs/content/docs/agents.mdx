...
global scope. To target Codex directly:

```bash
npx skills add https://github.com/floklein/natui --skill natui --agent codex
```

Codex users can also ask the built-in skill installer:

```text
$skill-installer install https://github.com/floklein/natui/tree/main/skills/natui
```

That last line is a Codex prompt, not a shell command. Start a new agent task
after installation so the skill is discovered. Review the
