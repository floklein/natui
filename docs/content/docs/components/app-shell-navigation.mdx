...
The host owns toolbar search text and reports it through `onSearch`. Both hosts
...
`TabView` takes its controlled value from a child `Tab` id. A tab click updates native
selection optimistically, then reports the id through `onChange`. Tab titles,
system images, and badges render on both hosts.

## Verification note

The kitchen-sink real-window suite exercises these seven app-shell kinds on
macOS and Windows.
