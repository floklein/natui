...
`@natui/core` and `create-natui-app` are registry packages. Outside a
checkout, the runtime downloads the release's prebuilt host on first launch.
Inside a checkout, the bridge only uses the host built from these sources, so
a stale download can never shadow your build; set `NATUI_HOST` to point an
application elsewhere at your build. The application packager remains a
repository-local reference workflow.
