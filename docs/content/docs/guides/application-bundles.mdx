...
- Windows x64:
  `NatUIDemo-<version>-windows-x64.exe`
- Windows ARM64:
  `NatUIDemo-<version>-windows-arm64.exe`
- macOS:
  `NatUIDemo.app`
...
Packaging fails on unknown configuration fields rather than ignoring them.
...
- **Windows:** Provide a valid multi-image `.ico` file. For broad
  ...
  representations. Prefer a transparent background. The packager
...
the current user's temporary directory at launch. The runtime manages the
extraction and installs nothing machine-wide. Copy the one EXE to ship the
application.
...
On macOS it then launches the `.app` through LaunchServices, requires a rendered
window screenshot, and verifies the same native close path. It needs a normal
