...
Checks `NATUI_HOST` first, then searches upward from the current directory for known macOS or Windows build outputs, then the per-user download cache. Synchronous; it never downloads.
