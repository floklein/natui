...
owns the rollup / Babel / esbuild toolchain, which a shipped application has no
reason to install; `natui dev` loads it on demand and tells you to install it if
it is missing. The command chooses its entry in this
order: a positional argument, `entry` from `natui.app.json`, then
`src/main.tsx`. Programmatic callers pass the resolved entry directly.
